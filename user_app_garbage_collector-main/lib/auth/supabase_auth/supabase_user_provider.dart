...
class GarbageCollectorSupabaseUser extends BaseAuthUser {
  GarbageCollectorSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> garbageCollectorSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = GarbageCollectorSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
